...
class StorageBase {
  static String path = '';
  static init() async {
    final databasesPath = Platform.isIOS
        ? ''
  ...
  static Future<DBMerge> connect() async {
    Database database = await openDatabase(
      ...
      onCreate: (Database db, int version) async {
        await db.execute(
          'CREATE TABLE data(id INTEGER PRIMARY KEY,name TEXT, text TEXT,type TEXT)',
        );
      },
    );
  ...
  static deleteAll(DatabaseCompute data) async {
    DBMerge db = await connect();
    await db.table('data').whereNotIn('name', [
      '_encrypted_session_master',
      '_encrypted_session_master_iv',
    ]).delete();
  }
  ...
  static Future<double?> getDouble(DatabaseCompute data) async {
    ...
    return list.isEmpty
        ? null
        : double.tryParse(_decrypt(data.encrypt, list.first['text']));
  }
  ...
  static Future<int?> getInt(DatabaseCompute data) async {
    ...
    return list.isEmpty
        ? null
        : int.tryParse(_decrypt(data.encrypt, list.first['text']));
  }
